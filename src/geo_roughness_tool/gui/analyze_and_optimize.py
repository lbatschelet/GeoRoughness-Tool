...
class AnalyzeAndOptimizeFrame(ctk.CTkFrame):
    def __init__(self, parent, main_gui, **kwargs):
        ...
        self.control_input_path_field = InputPathField(self, main_gui, "Training Data")
        self.control_input_path_field.grid(row=0,
                                           column=0,
                                           padx=(DEFAULTS.PADX, DEFAULTS.PADX * 0.5),
                                           pady=(DEFAULTS.PADY, DEFAULTS.PADY),
                                           sticky="nsew")

        # Right column, upper row: calculate quality button and label
        self.calculate_quality_frame = CalculationFrame(self, "Calculate Quality",
                                                        main_gui.calculate_quality,
                                                        label_prefix='Calculated quality of the thresholds (in %): ')
        self.calculate_quality_frame.grid(row=1,
                                          column=1,
                                          sticky="nsew",
        ...
        self.optimize_thresholds_frame = CalculationFrame(self,
                                                          "Optimize Thresholds",
                                                          main_gui.optimize_thresholds,
                                                          label_prefix='Calculated optimized thresholds: ')
        self.optimize_thresholds_frame.grid(row=0,
                                            column=1,
                                            sticky="nsew",
                                            padx=(DEFAULTS.PADX * 0.5, DEFAULTS.PADX),
                                            pady=(DEFAULTS.PADY * 1, DEFAULTS.PADY))

    def toggle_calculate_quality(self, show):
        if show:
            self.calculate_quality_frame.grid()
        else:
            self.calculate_quality_frame.grid_remove()
...
class CalculationFrame(ctk.CTkFrame):
    def __init__(self, parent, button_text, command, label_prefix="", **kwargs):
        ...
        self.calculation_button.grid(row=0,
                                     column=0,
                                     sticky="w",
                                     padx=(DEFAULTS.PADX * 0.5, DEFAULTS.PADX),
